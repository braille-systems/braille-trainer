            ...
            while 0 <= j < len(unit):  # для каждого шага юнита
                ...
                if isinstance(stp, LessonStep):
                    ...
                    while answ != stp.bLine and i < 3:
                        print('err')
                        if answ == '':
                            tts("простите, произошла ошибка распознавания. Повторите снова")
                            #playSoundByFilename('audio/std_msg/err_recognize.wav')
                        else:
                            tts("ответ неверный. Повторите заново после сигнала")
                    ...
                    if answ == stp.bLine:
                        stp.setRight()
                        print('ssess')
                        tts("Вы ответили верно. Это буква "+ stp.bLine)
                        #playSoundByFilename('audio/std_msg/ans_correct.wav')
                    else:
                        tts("Вы ответили неверно три раза. Правильный ответ: "+stp.bLine)
                ...
                print('joystick')
                joystick_ans = listen_serial(ser, "unit")
                print(joystick_ans)
                while joystick_ans:
                    if joystick_ans == 'r':
                        j = j + 1
...
def initMenu():
    ...
    U[1].append(LessonStep('', '%111111',
                           comment='Шрифт Брайля кодирует кажду букву комбинацией точек в ячейке Брайля. Точки расположены в два столбца, по три точки в столбце'))
    U[1].append(LessonStep('', '%100000',
                           comment='Для удобства точки пронумерованы от одного до шести. Точка 1 в левом верхнем углу, ознакомьтесь с ней'))
    U[1].append(
        LessonStep('', '%010000', comment='Дальше вниз идут точки 2 и 3. Потрогайте точку 2 - среднюю в левом столбце'))
    U[1].append(LessonStep('', '%001000', comment='Точка 3 - нижняя в левом столбце'))
    U[1].append(
        LessonStep('', '%000100', comment='В правом столбце сверху вниз идут точки 4, 5, 6. Сейчас выведена точка 4.'))
    ...
    U[3] = Unit(utype='test')
    U[3].title = 'Тест 1 - буквы А., Эм., У.'
    U[3].append(
        TestStep('', 'а', comment='Потрогайте букву на поверхности тренажёра. После сигнала произнесите её вслух'))
    U[3].append(
        TestStep('', 'м',
                 comment='Снова потрогайте букву на поверхности тренажёра. После сигнала произнесите её вслух'))
    U[3].append(TestStep('', 'а', comment='Потрогайте букву. После сигнала произнесите'))
    U[3].append(TestStep('', 'у', comment='Вновь произнесите выведенную букву после сигнала'))
    U[3].append(TestStep('', 'м', comment='ещё раз'))
    U[3].append(TestStep('', 'а', comment='и последний раз'))

    U[4] = Unit(utype='lesson')
    U[4].title = 'Урок 3. Буквы Ка, Эл.'
    U[4].append(LessonStep('', '', comment='Начнём с повторения.'))
    U[4].append(LessonStep('', 'мама', comment='мама'))
    U[4].append(LessonStep('', 'ума', comment='ума'))
    U[4].append(LessonStep('', 'ау', comment='ау'))
    U[4].append(LessonStep('', 'к',
                         comment='Перейдём к новому материалу. Перед вами буква К. Подумайте, где находятся точки, её составляющие? Какие они имеют номера?'))
    U[4].append(LessonStep('', 'к', comment='Ответ: точки 1 и 3, слева наверху и слева внизу, образуют букву к.'))
    U[4].append(LessonStep('', 'л', comment='Добавим к букве К точку номер 2. Сколько точек в полученной комбинации?'))
    U[4].append(LessonStep('', 'л',
                         comment='Тремя точками - все в левом столбце - обозначается буква Л. Запомните, Л - точки 1, 2, 3, расположены в форме столбика'))
    U[4].append(LessonStep('', 'лук', comment='лук'))
    U[4].append(LessonStep('', 'лама', comment='лама'))
    U[4].append(LessonStep('', 'кукла', comment='кукла'))
    U[4].append(LessonStep('', 'кулак', comment='кулак'))
